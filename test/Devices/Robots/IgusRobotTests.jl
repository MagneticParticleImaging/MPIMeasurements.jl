...
if ask_dialog("You are trying to start the Igus robot hardware test. Please ensure that the robot is turned on, is able to reference itself and can move at least 50mm in the positive direction. Please continue only if the robot is safe to move!","Cancel","Start test")
    if isReferenced(rob)
    ...
    @test state(rob)==:INIT
    @test dof(rob)==1
    @test namedPositions(rob)["origin"]==[0]u"mm"
    ...
    moveRel(rob, [10]u"mm")
    @test ask_dialog("Did the robot move 10mm in the positive direction?")
    moveRel(rob, 10u"mm")
    @test ask_dialog("Did the robot move 10mm in the positive direction?")
    ...
    moveAbs(rob, [20]u"mm")
    @test ask_dialog("Did the robot move to x=20mm?")
    @test getPosition(rob)==[20]u"mm"

    moveAbs(rob, 50u"mm")
    @test ask_dialog("Did the robot move to x=50mm?")
    @test getPosition(rob)==[50]u"mm"

    teachNamedPosition(rob, "pos1")
    @test issetequal(keys(namedPositions(rob)), ["origin", "pos1"])
    @test namedPositions(rob)["pos1"] == [50]u"mm"
    @test_throws RobotTeachError teachNamedPosition(rob, "pos1")

    moveAbs(rob, [10u"mm"],20u"mm/s")
    @test ask_dialog("Did the robot move faster to x=10mm?")
    @test getPosition(rob)==[10]u"mm"
    ...
    @test ask_dialog("Did the robot move faster to x=0mm?")

    moveAbs(rob, [50u"mm"],[20u"mm/s"])
    @test ask_dialog("Did the robot move faster to x=20mm?")
    @test getPosition(rob)==[50]u"mm"
