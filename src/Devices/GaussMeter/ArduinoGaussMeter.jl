...
function _init(gauss::ArduinoGaussMeter)
  ...
  sd = SerialDevice(sp, params.pause_ms, params.timeout_ms, "", "")
...
function initSerialPort(gauss::ArduinoGaussMeter, params::ArduinoGaussMeterDescriptionParams)
  pool = nothing
  if hasDependency(gauss, SerialPortPool)
    pool = dependency(gauss, SerialPortPool)
    sp = getSerialPort(pool, params.description, params.baudrate, ndatabits=params.ndatabits,parity=params.parity,nstopbits=params.nstopbits)
    if isnothing(sp)
      throw(ScannerConfigurationError("Device $(deviceID(gauss)) found no fitting serial port."))
    end
    return sp
